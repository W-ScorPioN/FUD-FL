...
from models.Update import DatasetSplit, Adult_dataloader, load_tensor, Celeba_Dataset
from utils.info import get_bias, save_info, save_models
...
if __name__ == '__main__':
    ...
    # CKA_time = [0 for j in range(args.num_run)]
    ...
    for i in range(args.num_run):
        ...
        local_model_each_epoch = [] # 保存每个epoch的本地模型，二维数组
        global_model_each_epoch = [] # 保存每个epoch的全局模型，一维数组
        w_locals = {}
        acc = []
        loss1 = []
        cv_loss, cv_acc = [], []
        ...
        for iter in range(args.epochs):
            ...
            local_model_each_epoch.append(w_locals) # 每个epoch的所有本地模型
            ...
            global_model_each_epoch.append(copy.deepcopy(w_glob)) # 每个epoch的所有全局模型
            # copy weight to net_glob
            net_glob.load_state_dict(w_glob)

            ldr_train_new = DataLoader(DatasetSplit(dataset_train, dict_users[0]), batch_size=args.local_bs, shuffle=True)
            # for images, labels in ldr_train_new:
            #     sample_train_data_local = images.to(args.device)
            #     break
            # net_glob.load_state_dict(w_glob)
            # file_name1 = f"res/glob_round_{iter}.pth"
            # torch.save(w_glob, file_name1)
            # baseline_model = net_glob
            # local_models = []
            # for i in range(m):
            #     model_new = copy.deepcopy(net_glob)
            #     model_new.load_state_dict(w_locals[i])
            #     local_models.append(model_new)
            #     file_name2 = f"res/client_{i}_round_{iter}.pth"
            #     torch.save(w_locals[i], file_name2)
            # CKA_compute_time_start = time.time()
            # similarities = compute_linear_CKA(baseline_model, local_models, sample_train_data_local)
            # CKA_compute_time_end = time.time()
            # cka_time1 += (CKA_compute_time_end - CKA_compute_time_start)

            # logging.info(f"sim value:{similarities}")
        ...
        agg[i] = agg_time
        # CKA_time[i] = cka_time1 / args.epochs
        # logging.info(f"num_run:{i}, CKA time:{CKA_time[i]}")
        ...
        # save_info(CKA_time, './save/FL/CKA_time/', args, i)
        save_models(local_model_each_epoch, './save/FL/local_models/', args, i) # 保存每个epoch的所有本地模型
    ...
    # get_bias('cka_time', CKA_time, args)
